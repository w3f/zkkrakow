...
impl AvailabilityBackend {
    async fn initialze(availablity_mnemonic: String) -> AvailabilityBackend {
	let phrase = bip39::Mnemonic::parse("load fame ill obtain abandon original demand film nasty account excuse bleak").expect("valid phrase expected");
    ...
    async fn submit_data(&self, data: Vec<u8>) -> H256 {
    ...
    async fn retrieve_data(&self, block_hash: H256) -> Vec<u8> {
...
mod tests {
	...
	let test_availablity_net = AvailabilityBackend::initialze("worth apart cage head yard argue usage guilt cigar sting flag dance".to_string());
	...
	let test_availablity_net = AvailabilityBackend::initialze("worth apart cage head yard argue usage guilt cigar sting flag dance".to_string()).await;
